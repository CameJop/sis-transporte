                ...
                <Column header="REMITENTE / DESTINATARIO">
                    <template #body="{ data }">
                        <div class="flex flex-col gap-1">
                            ...
                            <span class="text-white text-xs font-semibold">
                                <i class="pi pi-download text-[10px] text-slate-500 mr-1"></i> {{
                ...
                <Column header="VIAJE / RUTA">
                    <template #body="{ data }">
                        <div class="text-xs text-slate-300">
                            {{ data.viaje?.itinerario?.ruta?.origen?.nombre }} ➔ {{
                                data.viaje?.itinerario?.ruta?.destino?.nombre }}
                        </div>
                        <div class="text-[10px] text-indigo-400 font-bold mt-1 uppercase font-mono">
                            Salida: {{ data.viaje?.fecha_salida }} | {{ data.viaje?.itinerario?.hora_salida }}
                        </div>
                    </template>
                </Column>
                <Column header="DETALLES">
                    <template #body="{ data }">
                        <Tag :value="`${data.peso} Kg`"
                            class="!bg-white/5 !text-slate-300 !border !border-white/10 font-mono" />
                    </template>
                </Column>
            ...
            <form @submit.prevent="submit" class="grid grid-cols-1 gap-5 pt-2">
                <div class="flex flex-col gap-2">
                    ...
                    <Dropdown v-model="form.id_viaje" :options="viajes"
                        :optionLabel="(v) => `${v.itinerario?.ruta?.origen?.nombre} ➔ ${v.itinerario?.ruta?.destino?.nombre} | ${v.fecha_salida} ${v.itinerario?.hora_salida} (${v.bus?.placa})`"
                        optionValue="id_viaje" :appendTo="'self'" filter placeholder="Seleccione el viaje"
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div class="flex flex-col gap-2">
                        ...
                        <Dropdown v-model="form.id_cliente_remitente" :options="clientes" optionLabel="nombre"
                            optionValue="id_cliente" :appendTo="'self'" filter placeholder="Remitente"
                            class="w-full !bg-[#0f111a] !border-white/10" />
                    </div>
                    <div class="flex flex-col gap-2">
                        <label class="text-xs font-bold text-slate-500 uppercase tracking-widest">Destinatario</label>
                        <Dropdown v-model="form.id_cliente_destinatario" :options="clientes" optionLabel="nombre"
                            optionValue="id_cliente" :appendTo="'self'" filter placeholder="Destinatario"
                            class="w-full !bg-[#0f111a] !border-white/10" />
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div class="flex flex-col gap-2">
                        ...
                        <InputNumber v-model="form.peso" mode="decimal" :minFractionDigits="2" class="w-full"
                            inputClass="!bg-[#0f111a] !border-white/10 !text-white" />
                    </div>
                    <div class="flex flex-col gap-2">
                ...
                <div class="flex flex-col gap-2">
                    ...
                    <Dropdown v-model="form.estado" :options="estadosEncomienda" optionLabel="label" optionValue="value"
                        :appendTo="'self'" class="w-full !bg-[#0f111a] !border-white/10" />
                </div>

                <div class="flex justify-end gap-3 mt-4">
                    <Button label="Cerrar" text severity="secondary" @click="displayModal = false"
                        class="!text-slate-400" />
                    <Button :label="editMode ? 'Actualizar' : 'Registrar Envío'" type="submit" icon="pi pi-check"
